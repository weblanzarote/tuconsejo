...
const notificationsRouter = router({
  getSettings: protectedProcedure.query(async ({ ctx }) => {
    const s = await getNotificationSettings(ctx.user.id);
    if (!s) {
      return { userId: ctx.user.id, ...DEFAULT_NOTIFICATION_SETTINGS };
    }
    return s;
  }),
  updateSettings: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.telegramChatId !== undefined) {
        patch.telegramChatId = input.telegramChatId?.trim() || null;
      }
      return upsertNotificationSettings(ctx.user.id, patch);
    }),
  sendTest: protectedProcedure.mutation(async ({ ctx }) => {
    const s = await getNotificationSettings(ctx.user.id);
    if (!s?.telegramChatId) {
      throw new TRPCError({ code: "BAD_REQUEST", message: "Configura tu chat_id de Telegram primero" });
    }
    const text = `✅ <b>Consejo Sinérgico</b>\nNotificaciones de Telegram activas para ${escapeTelegramHtml(
      ctx.user.name ?? ctx.user.username
    )}.`;
    const res = await sendTelegramMessage(s.telegramChatId, text);
    if (!res.ok) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: res.error ?? "Error enviando" });
    }
    return { success: true } as const;
  }),
  flushNow: protectedProcedure.mutation(async ({ ctx }) => {
    const sent = await flushUserQueue(ctx.user.id);
    return { sent };
  }),
});
